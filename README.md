...
└── final_metrics.json  retrieval/clustering/embedding metrics + run config snapshot
```
...
| `pca_class`  | `pca_latent_space_class.png`  | post-hoc inference + COCO single-object class filter |
...
`pca_class` is a variant of `pca` that **colors points by COCO object category and shapes by modality** (image=square `s`, text=star `*`, marker convention borrowed from TempModGap). Class labels come from `instances_val2017.json` via the same single-object filter used by the clustering eval — only val2017 images whose annotations contain exactly one distinct category survive (≈15-18% of the requested samples). Useful for verifying that text and image embeddings of the same semantic class converge to the same region of the sphere as training progresses (paper Figure 2 style). COCO only.
...
python Code/ModalityGap/visualization.py --run_name <run_name> --plots pca pca_class \
    --pca_num_samples 5000 --pca_epochs 0 20 50 100
